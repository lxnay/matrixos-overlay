...
src_prepare() {
	default
	test -e "${S}/contrib/kernel-install/91-sbctl.install" || die
	einfo "Fixing 91-sbctl.install for kernel-install ..."
	cp "${FILESDIR}/91-sbctl.install" "${S}/contrib/kernel-install/91-sbctl.install"
}
