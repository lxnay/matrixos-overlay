...
src_prepare() {
	...
	einfo "Copying matrixOS plymouthd.conf"
	cp "${FILESDIR}/plymouthd.conf" "${S}/src/plymouthd.conf" || die
}
...
src_install() {
	...
	# matrixOS logo provided by matrixos-artwork-plymouth
	rm "${D}"/usr/share/plymouth/bizcom.png || die
}
