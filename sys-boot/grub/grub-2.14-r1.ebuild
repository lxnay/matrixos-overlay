...
SLOT="2/${PVR}"
IUSE="+branding +device-mapper doc efiemu +fonts mount nls protect +sbat4 sdl test +themes truetype libzfs"
...
"
RDEPEND="${DEPEND}
	branding? ( sys-boot/matrixos-artwork-grub )
	kernel_linux? (
...
src_install() {
	...
	if use branding && use themes ; then
		sed -i -e 's:^#GRUB_THEME=.*$:GRUB_THEME="/boot/grub/themes/matrixos-theme/theme.txt":g' \
